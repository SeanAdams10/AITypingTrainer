...
class Word:
    ...
    def type_char(self, char: str) -> bool:
        ...
        if (
            self.typed_chars < len(self.text)
            and self.text[self.typed_chars].lower() == char.lower()
        ):
            self.typed_chars += 1
...
class SpaceInvadersGame(QDialog):
    ...
    def __init__(self, parent: Optional[QWidget] = None) -> None:
        """Initialize the game dialog and set up timers, state, and UI."""
        ...
        if self.current_target:
            if self.current_target.type_char(key):
                ...
                if self.current_target.is_complete:
                    ...
                    self.score += len(self.current_target.original_text)
                    self.score_label.setText(f"Score: {self.score}")
    ...
    def find_target_word(self, char: str) -> Optional[Word]:
        ...
        candidates = [
            word for word in self.words if not word.is_complete and word.starts_with(char)
        ]
