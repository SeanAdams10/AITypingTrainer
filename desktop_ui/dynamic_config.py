...
class DynamicConfigDialog(QDialog):
    ...
    def __init__(
        ...
        if self.db_manager:
            ...
            self.setting_manager = global_setting_manager
    ...
    def _setup_ui(self) -> None:
        ...
        self.speed_radio = QRadioButton("Focus on Speed")
        self.accuracy_radio = QRadioButton("Focus on Accuracy")
        ...
        self.focus_on_speed_target = QCheckBox("Focus on speed target (only slower than target)")
        ...
        self.top_ngrams_count = QSpinBox()
        ...
        self.pure_ngram_radio = QRadioButton("Pure N-gram")
        ...
        practice_type_layout = QHBoxLayout()
        ...
        self.ngram_table.setEditTriggers(QAbstractItemView.EditTrigger.NoEditTriggers)

        analysis_layout.addWidget(self.ngram_table)

        # Generate button
        self.generate_btn = QPushButton("Generate Practice Content")
        ...
        button_box.addButton(self.metroid_game_btn, QDialogButtonBox.ButtonRole.AcceptRole)
            ...
            if focus_on_speed:
                ...
                if ngram_stats:
                    ...
                    for row, stats in enumerate(ngram_stats):
                        ...
                        self.ngram_table.setItem(
                            row, 3, QTableWidgetItem(f"{stats.ngram_score:.2f}")
                        )
                else:
                ...
                if ngram_stats:
                    ...
                    for row, stats in enumerate(ngram_stats):
                        self.ngram_table.setItem(row, 0, QTableWidgetItem(stats.ngram))
                        self.ngram_table.setItem(
                            row, 1, QTableWidgetItem(f"{stats.total_occurrences}")
                        )
                else:
                    # Show message when no data is available
                    self.ngram_table.setRowCount(1)
    ...
    def _load_settings(self) -> None:
        ...
        try:
            ...
            entry = global_setting_cache.get("NGRSZE", self.keyboard_id)
            ...
            entry = global_setting_cache.get("NGRCNT", self.keyboard_id)
            if entry and entry.setting.setting_value:
            ...
            entry = global_setting_cache.get("NGRMOC", self.keyboard_id)
            if entry and entry.setting.setting_value:
            ...
            entry = global_setting_cache.get("NGRLEN", self.keyboard_id)
            if entry and entry.setting.setting_value:
            ...
            entry = global_setting_cache.get("NGRKEY", self.keyboard_id)
            if entry and entry.setting.setting_value:
            ...
            entry = global_setting_cache.get("NGRTYP", self.keyboard_id)
            if entry and entry.setting.setting_value:
            ...
            entry = global_setting_cache.get("NGRFST", self.keyboard_id)
            if entry and entry.setting.setting_value:
    ...
    def _save_settings(self) -> None:
        """Save settings via global_setting_cache using specific setting keys."""
        if not self.setting_manager or not self.keyboard_id:
            return
        ...
        try:
            ...
            global_setting_cache.set(
                "NGRSZE",
                self.keyboard_id,
                SettingCacheEntry(ngram_size_setting),
            )
            ...
            global_setting_cache.set(
                "NGRCNT",
                self.keyboard_id,
                SettingCacheEntry(ngrams_count_setting),
            )
            ...
            global_setting_cache.set(
                "NGRMOC",
                self.keyboard_id,
                SettingCacheEntry(min_occurrences_setting),
            )
            ...
            global_setting_cache.set(
                "NGRLEN",
                self.keyboard_id,
                SettingCacheEntry(practice_len_setting),
            )
            ...
            global_setting_cache.set(
                "NGRKEY",
                self.keyboard_id,
                SettingCacheEntry(included_keys_setting),
            )
            ...
            global_setting_cache.set(
                "NGRTYP",
                self.keyboard_id,
                SettingCacheEntry(practice_type_setting),
            )
            ...
            global_setting_cache.set(
                "NGRFST",
                self.keyboard_id,
                SettingCacheEntry(focus_on_speed_target_setting),
            )
