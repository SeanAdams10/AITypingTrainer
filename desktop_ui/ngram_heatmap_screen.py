...
class NGramHeatmapDialog(QtWidgets.QDialog):
    ...
    def setup_control_panel(self, parent_layout: QtWidgets.QVBoxLayout) -> None:
        ...
        self.speed_max_spin.setValue(1000)
    ...
    def setup_heatmap_area(self, parent_layout: QtWidgets.QVBoxLayout) -> None:
        ...
        for label, color in legend_colors:
            ...
            legend_item.setStyleSheet(
                f"background-color: {color}; padding: 2px 8px; "
    ...
    def apply_filters(self) -> None:
        ...
        for item in self.heatmap_data:
            # Speed filter
            if item.decaying_average_ms < min_speed or item.decaying_average_ms > max_speed:
                continue
            
            # N-gram size filter
            if ngram_size is not None and item.ngram_size != ngram_size:
                continue
    ...
    def export_data(self) -> None:
        ...
        if not self.filtered_data:
            QtWidgets.QMessageBox.information(
                self, "No Data", "No data to export."
            )
            return
        
        # Get save file path
        default_filename = (
            f"ngram_heatmap_{self.user.user_id}_{self.keyboard.keyboard_id}.csv"
        )
        ...
        try:
            ...
            with open(file_path, 'w', newline='', encoding='utf-8') as csvfile:
                ...
                # Write header
                writer.writerow([
                    'N-gram', 'Size', 'Average Speed (ms)', 'Target Speed (ms)',
                    'Meets Target', 'Sample Count', 'Last Updated'
                ])
                ...
                for item in self.filtered_data:
                    writer.writerow([
                        ...
                        item.sample_count,
                        item.last_updated.strftime('%Y-%m-%d %H:%M:%S') if item.last_updated else "N/A"
                    ])
            
            self.status_label.setText(f"Data exported to {file_path}")
            QtWidgets.QMessageBox.information(
                self, "Export Complete", f"Data exported successfully to {os.path.basename(file_path)}"
            )
        except Exception as e:
    ...
    def update_heatmap_display(self) -> None:
        ...
        if sort_by == "N-gram Text":
            ...
            self.filtered_data.sort(
                key=lambda x: perf_order.get(x.performance_category, 4)
            )
