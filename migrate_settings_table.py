...
Legacy migration script to update the settings table to the new schema.
This script was for SQLite databases only.
"""
...
def get_sample_users(use_cloud: bool = True, limit: int = 10) -> List[Tuple]:
    ...
    try:
        db_manager = DatabaseManager(
            db_path=None, connection_type=connection_type, debug_util=debug_util
        )
...
def calculate_setting_checksum(
    setting_type_id: str, setting_value: str, related_entity_id: str
) -> str:
...
def migrate_settings_table(default_user_id: str, use_cloud: bool = True) -> bool:
    ...
    try:
        ...
        if "created_user_id" not in existing_columns:
            ...
            cursor.execute(
                f'ALTER TABLE settings ADD COLUMN created_user_id TEXT DEFAULT "{default_user_id}"'
            )
            cursor.execute(
                f'UPDATE settings SET created_user_id = "{default_user_id}" WHERE created_user_id IS NULL'
            )

        if "updated_user_id" not in existing_columns:
            print("Adding updated_user_id column...")
            cursor.execute(
                f'ALTER TABLE settings ADD COLUMN updated_user_id TEXT DEFAULT "{default_user_id}"'
            )
            cursor.execute(
                f'UPDATE settings SET updated_user_id = "{default_user_id}" WHERE updated_user_id IS NULL'
            )

        if "created_at" not in existing_columns:
            print("Adding created_at column...")
            cursor.execute(
                f'ALTER TABLE settings ADD COLUMN created_at TEXT DEFAULT "{current_time}"'
            )
            cursor.execute(
                f'UPDATE settings SET created_at = "{current_time}" WHERE created_at IS NULL'
            )
        ...
        if "row_checksum" not in existing_columns:
            ...
            cursor.execute(
                "SELECT rowid, setting_type_id, setting_value, related_entity_id FROM settings"
            )
            records = cursor.fetchall()

            for record in records:
                rowid, setting_type_id, setting_value, related_entity_id = record
                checksum = calculate_setting_checksum(
                    setting_type_id, setting_value, related_entity_id
                )
                cursor.execute(
                    "UPDATE settings SET row_checksum = ? WHERE rowid = ?", (checksum, rowid)
                )
...
def verify_migration(use_cloud: bool = True) -> bool:
    ...
    try:
        ...
        cursor.execute(
            "SELECT COUNT(*) FROM settings WHERE created_user_id IS NULL OR updated_user_id IS NULL OR created_at IS NULL OR row_checksum IS NULL"
        )
        ...
        cursor.execute(
            "SELECT setting_type_id, setting_value, related_entity_id, row_checksum FROM settings LIMIT 5"
        )
        sample_records = cursor.fetchall()

        for record in sample_records:
            setting_type_id, setting_value, related_entity_id, stored_checksum = record
            calculated_checksum = calculate_setting_checksum(
                setting_type_id, setting_value, related_entity_id
            )
...
def main():
    ...
    if success:
        ...
        if verify_migration(use_cloud):
            ...
            print(
                "2. If everything works, you can drop the backup table: DROP TABLE settings_backup_pre_migration"
            )
            return True
        else:
