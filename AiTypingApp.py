...
from PyQt5.QtWidgets import (
    ...
    QPushButton, QWidget, QDialog, QMessageBox, QFrame, 
...
API_SCRIPT_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), "api", "run_library_api.py")
...
class ServerWorker(QThread):
    ...
    def __init__(self, server_type: str, script_path: Optional[str] = None, 
                 cwd: Optional[str] = None, npm_command: Optional[List[str]] = None) -> None:
        """
    ...
    def run(self) -> None:
        ...
        try:
            if self.server_type == "api" and self.script_path:
                ...
                try:
                    response = requests.post(API_SERVER_URL, json={"query": "{__schema{types{name}}}"})
                    if response.status_code == 200:
                        self.is_running = True
                        self.status_update.emit("GraphQL API server running")
                        self.server_started.emit(True)
                    else:
                        self.status_update.emit(f"API server returned status {response.status_code}")
...
class SplashScreen(QDialog):
    ...
    def check_api_server(self) -> None:
        ...
        try:
            response = requests.post(API_SERVER_URL, json={"query": "{__schema{types{name}}}"}, timeout=2)
            if response.status_code == 200:
