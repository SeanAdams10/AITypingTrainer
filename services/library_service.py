...
class Category(Base):
    ...
    snippets = relationship(
        "Snippet", back_populates="category", cascade="all, delete-orphan"
    )
...
class Snippet(Base):
    ...
    __table_args__ = (
        UniqueConstraint("category_id", "name", name="uix_category_snippet_name"),
    )
...
class LibraryService:
    ...
    def edit_snippet(self, snippet_id, new_name, new_content, new_category_id=None):
        ...
        category_id = (
            new_category_id if new_category_id is not None else snip.category_id
        )
        if (
            self.session.query(Snippet)
            .filter(
