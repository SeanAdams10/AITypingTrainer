...
"""
from typing import List, Optional, Tuple, Union, TypedDict # Removed Dict, Any
import datetime
import sqlite3
import traceback
    ...
    def get_keystrokes_for_session(self, session_id: str) -> List[KeystrokeFromDB]:
        ...
        for i, row in enumerate(rows):
            result.append({
                ...
                'time_since_previous': row['time_since_previous'] if row['time_since_previous'] is not None else 0,
...
class NgramAnalyzer:
    ...
    def _convert_timestamp_to_ms(self, ts_val: Union[str, datetime.datetime, int, float, None]) -> float:
    ...
    def analyze_session_ngrams(self, 
        ...
        try:
            ...
            print(f"Processing content: '{content}' with {len(keystrokes)} keystrokes for session {session_id}")
            ...
            sid_log = session_id_for_log or (keystrokes[0]['session_id'] if keystrokes else "unknown")
            ...
            try:
                if content and session_id_for_log: # Use already fetched/validated values
                    ...
                    s_id_fallback = keystrokes[0]['session_id'] if keystrokes and 'session_id' in keystrokes[0] else None
                    if s_id_fallback and content: # content might still be from a partially successful try block
                         self._create_default_ngram_records(s_id_fallback, content)
                    elif s_id_fallback and not content: # Try to fetch content if missing for default records
                        session_query_fallback = "SELECT content FROM practice_sessions WHERE session_id = ?"
                        session_row_fallback = self.db_manager.execute(session_query_fallback, (s_id_fallback,)).fetchone()
                        if session_row_fallback and session_row_fallback['content']:
                            self._create_default_ngram_records(s_id_fallback, session_row_fallback['content'])
                        else:
                            print(f"Cannot create default n-gram records due to missing session_id or content during error handling for session {sid_log}.")
                    else:
                        print(f"Cannot create default n-gram records due to missing session_id or content during error handling for session {sid_log}.")

            except Exception as inner_e:
                print(f"Error creating default n-gram records during outer exception handling for session {sid_log}: {inner_e}")
            return False
    ...
    def _analyze_ngram_speed(self, 
        ...
        try:
            for i in range(len(content) - ngram_size + 1):
                ...
                if start_idx >= len(keystrokes) or end_idx > len(keystrokes) or start_idx >= end_idx:
                    continue
                
                ngram_keystrokes = keystrokes[start_idx:end_idx]
                
                if not ngram_keystrokes or len(ngram_keystrokes) < ngram_size: # Ensure enough keystrokes for the ngram
                    continue
                
                # Timestamps are datetime.datetime objects in KeystrokeForNgramAnalysis
                start_time_val = ngram_keystrokes[0]['timestamp']
                end_time_val = ngram_keystrokes[-1]['timestamp'] # ngram_keystrokes is at least of ngram_size
                ...
                if duration < 0: duration = 0 # Guard against clock issues or very fast typing
                ...
                if len(ngram_keystrokes) > 1:
                    ...
                    if actual_duration == 0 and len(ngram_keystrokes) > 1 : # if all time_since_previous are 0
            ...
            print(f"Error analyzing ngram speed for session {session_id}, ngram '{ngram if ngram else 'unknown'}': {e}")
            traceback.print_exc()
    ...
    def _analyze_ngram_errors(self, 
        ...
        try:
            ...
            print(f"Error analyzing ngram errors for session {session_id}, ngram '{ngram if ngram else 'unknown'}': {e}")
            traceback.print_exc()

def _simulate_keystroke_timing(char_typed: str, previous_char: Optional[str], base_delay_ms: int = 120) -> int:
    """
...
def _generate_realistic_keystrokes(session_id: str, text_to_type: str, error_rate: float = 0.05, wpm: int = 40) -> List[KeystrokeInputData]:
    """
    ...
    avg_delay_ms = 1000 / chars_per_second if chars_per_second > 0 else 120 # Default to 120ms if WPM is 0
    ...
    for i, char_expected in enumerate(text_to_type):
        ...
        if i == 0:
            ...
            base_sim_delay = _simulate_keystroke_timing(char_expected, previous_char, base_delay_ms=int(avg_delay_ms))
        ...
        if error_rate > 0 and random.random() < error_rate:
            ...
            char_to_type = random.choice(possible_errors) if possible_errors else '~' # Fallback if no other choice
...
def simulate_typing_session_and_save(
    ...
    if success:
        print(f"Session {session_id} saved successfully with {len(generated_keystrokes)} keystrokes.")
...
def save_session_data(
    ...
    try:
        ...
        total_keystrokes_for_accuracy = sum(1 for ks in keystrokes_input if ks['char_typed'] != '\b')
        accuracy = (correct_keystrokes / total_keystrokes_for_accuracy) * 100 if total_keystrokes_for_accuracy > 0 else 0.0
        ...
        relevant_times = [
            ks['time_since_previous'] for ks in keystrokes_input 
