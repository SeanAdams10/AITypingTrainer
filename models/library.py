...
class LibraryManager:
    ...
    def list_categories(self) -> List[LibraryCategory]:
        rows = self.db.fetchall("SELECT category_id, category_name FROM text_category ORDER BY category_name")
    ...
    def create_category(self, name: str) -> int:
        ...
        if self.db.fetchone("SELECT 1 FROM text_category WHERE category_name = ?", (name,)):
    ...
    def rename_category(self, category_id: int, new_name: str) -> None:
        ...
        if not self.db.fetchone("SELECT 1 FROM text_category WHERE category_id = ?", (category_id,)):
            raise CategoryNotFoundError(f"Category {category_id} does not exist.")
        if self.db.fetchone("SELECT 1 FROM text_category WHERE category_name = ? AND category_id != ?", (new_name, category_id)):
            raise CategoryExistsError(f"Category '{new_name}' already exists.")
        self.db.execute(
            "UPDATE text_category SET category_name = ? WHERE category_id = ?",
            (new_name, category_id), commit=True
        )
    ...
    def delete_category(self, category_id: int) -> None:
        if not self.db.fetchone("SELECT 1 FROM text_category WHERE category_id = ?", (category_id,)):
            raise CategoryNotFoundError(f"Category {category_id} does not exist.")
        # Cascade delete
        self.db.execute("DELETE FROM snippet_parts WHERE snippet_id IN (SELECT snippet_id FROM text_snippets WHERE category_id = ?)", (category_id,), commit=True)
        self.db.execute("DELETE FROM text_snippets WHERE category_id = ?", (category_id,), commit=True)
        self.db.execute("DELETE FROM text_category WHERE category_id = ?", (category_id,), commit=True)

    # SNIPPET CRUD
    def list_snippets(self, category_id: int) -> List[LibrarySnippet]:
        rows = self.db.fetchall("SELECT snippet_id, category_id, snippet_name, content FROM text_snippets WHERE category_id = ? ORDER BY snippet_name", (category_id,))
        return [LibrarySnippet(snippet_id=row[0], category_id=row[1], snippet_name=row[2], content=row[3]) for row in rows]
    ...
    def create_snippet(self, category_id: int, snippet_name: str, content: str) -> int:
        ...
        if self.db.fetchone("SELECT 1 FROM text_snippets WHERE category_id = ? AND snippet_name = ?", (category_id, snippet_name)):
        ...
        cur = self.db.execute(
            ...
            (category_id, snippet_name, content), commit=True
        )
        snippet_id = cur.lastrowid
        self._split_and_save_parts(snippet_id, content)
        return snippet_id

    def edit_snippet(self, snippet_id: int, snippet_name: str, content: str, category_id: Optional[int] = None) -> None:
        snippet_name = LibrarySnippet.validate_name(snippet_name)
        content = LibrarySnippet.validate_content(content)
        row = self.db.fetchone("SELECT category_id FROM text_snippets WHERE snippet_id = ?", (snippet_id,))
        if not row:
            raise SnippetNotFoundError(f"Snippet {snippet_id} does not exist.")
        old_category_id = row[0]
        if self.db.fetchone("SELECT 1 FROM text_snippets WHERE category_id = ? AND snippet_name = ? AND snippet_id != ?", (category_id or old_category_id, snippet_name, snippet_id)):
            raise SnippetExistsError(f"Snippet '{snippet_name}' already exists in this category.")
        # Update snippet
        self.db.execute(
            "UPDATE text_snippets SET snippet_name = ?, content = ?, category_id = ? WHERE snippet_id = ?",
            (snippet_name, content, category_id or old_category_id, snippet_id), commit=True
        )
        # Re-split parts
        self.db.execute("DELETE FROM snippet_parts WHERE snippet_id = ?", (snippet_id,), commit=True)
        self._split_and_save_parts(snippet_id, content)

    def delete_snippet(self, snippet_id: int) -> None:
        if not self.db.fetchone("SELECT 1 FROM text_snippets WHERE snippet_id = ?", (snippet_id,)):
            raise SnippetNotFoundError(f"Snippet {snippet_id} does not exist.")
        self.db.execute("DELETE FROM snippet_parts WHERE snippet_id = ?", (snippet_id,), commit=True)
        self.db.execute("DELETE FROM text_snippets WHERE snippet_id = ?", (snippet_id,), commit=True)

    # SNIPPET PARTS
    def list_parts(self, snippet_id: int) -> List[SnippetPart]:
        rows = self.db.fetchall("SELECT part_id, snippet_id, part_number, content FROM snippet_parts WHERE snippet_id = ? ORDER BY part_number", (snippet_id,))
        return [SnippetPart(part_id=row[0], snippet_id=row[1], part_number=row[2], content=row[3]) for row in rows]
    ...
    def _split_and_save_parts(self, snippet_id: int, content: str) -> None:
        ...
        for idx, part in enumerate(parts, 1):
            self.db.execute(
                ...
                (snippet_id, idx, part), commit=True
            )
