...
class Keystroke:
    ...
    def save(self) -> bool:
        ...
        try:
            ...
            if self.session_id is None or not isinstance(self.session_id, int):
                print(f"Error: Invalid or missing session_id ({self.session_id}) for keystroke save.", file=sys.stderr)
                return False
            ...
            if self.keystroke_id is None:
                ...
                try:
                    ...
                    print(f"Error: Invalid keystroke_id type ({type(self.keystroke_id)}) for keystroke save.", file=sys.stderr)
                    return False
            ...
            success = db.execute_update(
                ...
                (
                    self.session_id,
                    self.keystroke_id,
                    self.keystroke_time.isoformat() if hasattr(self.keystroke_time, 'isoformat') else self.keystroke_time,
                    self.keystroke_char,
                    self.expected_char,
                    int(self.is_correct),
    ...
    def save_many(cls, session_id: int, keystrokes: List[Dict[str, Any]]) -> bool:
        ...
        if not isinstance(session_id, int) or not keystrokes:
            print(f"Error: Invalid session_id ({session_id}) or empty keystrokes list in save_many.", file=sys.stderr)
            return False
        ...
        try:
            ...
            cursor.execute("SELECT MAX(error_id) FROM practice_session_errors WHERE session_id = ?", (session_id,))
            ...
            cursor.execute("SELECT MAX(keystroke_id) FROM practice_session_keystrokes WHERE session_id = ?", (session_id,))
            ...
            for k_data in keystrokes:
                ...
                if k_id is None:
                    ...
                    try:
                        ...
                        print(f"Warning: Skipping keystroke with invalid keystroke_id: {k_data.get('keystroke_id')}", file=sys.stderr)
                        continue
