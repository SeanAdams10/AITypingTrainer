...
def patch_category_dialog(monkeypatch):
    ...
    class FakeCategoryDialog(CategoryDialog):
        def __init__(self, title: str, label: str, default: str = "", parent=None) -> None:
...
def patch_snippet_dialog(monkeypatch):
    ...
    class FakeSnippetDialog(SnippetDialog):
        def __init__(self, title: str, name_label: str, content_label: str, default_name: str = "", default_content: str = "", parent=None) -> None:
            super().__init__(title, name_label, content_label, default_name, default_content, parent)
...
def test_add_category(main_window: LibraryMainWindow, qtbot):
    ...
    assert main_window.status.text().startswith("Category 'TestCategory' added successfully")
...
def test_add_snippet(main_window: LibraryMainWindow, qtbot):
    ...
    assert main_window.status.text().startswith("Snippet 'TestSnippet' added successfully")
...
def test_edit_snippet(main_window: LibraryMainWindow, qtbot, monkeypatch):
    ...
    assert any(s["snippet_name"] == "RenamedSnippet" for s in main_window.snippets), "Renamed snippet not found in main_window.snippets"
...
def test_delete_snippet(main_window: LibraryMainWindow, qtbot, monkeypatch):
    ...
    assert main_window.snippetList.count() == 0, f"Expected 0 snippets, found {main_window.snippetList.count()}"
    # Don't assert the status text as it may be changed by load_snippets()
...
def test_delete_category(main_window: LibraryMainWindow, qtbot, monkeypatch):
    ...
    assert main_window.status.text() == "" or "deleted" in main_window.status.text().lower()
...
def test_add_empty_snippet(main_window: LibraryMainWindow, qtbot, monkeypatch):
    ...
    assert "Validation error" in error_text or "Error" in error_text, f"Expected validation error message, got: '{error_text}'"
