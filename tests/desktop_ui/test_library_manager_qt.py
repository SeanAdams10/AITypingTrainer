...
@pytest.fixture
def patch_qdialog_with_qtbot(qtbot, monkeypatch):
    original_init = QtWidgets.QDialog.__init__
    def new_init(self, *args, **kwargs):
        original_init(self)
        qtbot.addWidget(self)
    monkeypatch.setattr(QtWidgets.QDialog, "__init__", new_init)
    yield
    monkeypatch.setattr(QtWidgets.QDialog, "__init__", original_init)
...
# --- Snippet Tests ---
def test_add_snippet_valid(qtbot, app, monkeypatch, patch_qdialog_with_qtbot):
    # Add category
    monkeypatch.setattr(QtWidgets.QInputDialog, "getText", lambda *a, **k: ("Alpha", True))
    qtbot.mouseClick(app.btn_add_cat, QtCore.Qt.LeftButton)
    app.cat_list.setCurrentRow(0)
...
def test_search_snippet(qtbot, app, monkeypatch):
    ...
    qtbot.mouseClick(app.btn_add_cat, QtCore.Qt.LeftButton)
    app.cat_list.setCurrentRow(0)
    def make_dummy_dialog(qtbot, name):
        class DummyDialog(QtWidgets.QDialog):
            def __init__(self, *args, **kwargs):
                super().__init__()
                qtbot.addWidget(self)
            ...
            def exec_(self):
                ...
                return QtWidgets.QDialog.Accepted
        return DummyDialog
    monkeypatch.setattr("desktop_ui.library_manager.SnippetDialog", lambda *a, **k: make_dummy_dialog(qtbot, "S1")(*a, **k))
    qtbot.mouseClick(app.btn_add_snip, QtCore.Qt.LeftButton)
    monkeypatch.setattr("desktop_ui.library_manager.SnippetDialog", lambda *a, **k: make_dummy_dialog(qtbot, "S2")(*a, **k))
    qtbot.mouseClick(app.btn_add_snip, QtCore.Qt.LeftButton)
