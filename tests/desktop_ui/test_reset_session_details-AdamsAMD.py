...
def test_clear_all_session_data(populated_db):
    ...
    keystrokes_before = populated_db.execute("SELECT COUNT(*) FROM session_keystrokes").fetchone()[0]
    ...
    session_manager = PracticeSessionManager(populated_db)
...
def test_reset_sessions_confirmed(temp_db, qtapp, qtbot, mock_qtwidgets_confirm):
    ...
    assert db.execute("SELECT COUNT(*) FROM practice_sessions").fetchone()[0] == 0, "practice_sessions table should be empty after reset"
    assert db.execute("SELECT COUNT(*) FROM session_keystrokes").fetchone()[0] == 0, "session_keystrokes table should be empty after reset"
    
    # Check n-gram tables if they exist
    if 'session_ngram_speed' in table_names:
        assert db.execute("SELECT COUNT(*) FROM session_ngram_speed").fetchone()[0] == 0, "session_ngram_speed table should be empty after reset"
    
    if 'session_ngram_errors' in table_names:
        assert db.execute("SELECT COUNT(*) FROM session_ngram_errors").fetchone()[0] == 0, "session_ngram_errors table should be empty after reset"
...
def test_reset_sessions_cancelled(temp_db, qtapp, qtbot, mock_qtwidgets_cancel):
    ...
    assert sessions_after == sessions_before, "practice_sessions count should remain unchanged when reset is canceled"
    assert keystrokes_after == keystrokes_before, "session_keystrokes count should remain unchanged when reset is canceled"
