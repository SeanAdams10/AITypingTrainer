...
def create_keystroke(
    position: int, character: str, timestamp: float = 1.0, is_error: int = 0
) -> Dict[str, Any]:
...
def create_error_record(
    position: int, expected_char: str, actual_char: str, timestamp: float = 1.0
) -> Dict[str, Any]:
...
def pytest_configure(config):
    ...
    config.addinivalue_line(
        "markers", "populate_sessions: mark test as requiring session population"
    )
...
def mock_session_manager() -> SessionManager:
    """Create a mock SessionManager for testing session persistence.
    ...
    Returns:
        SessionManager: A mocked session manager that returns predefined values.
    """
...
def test_typing_drill_screen_initialization(app: QApplication, qtbot: Any) -> None:
    ...
    assert (content in display_text) or (preprocessed_content in display_text), (
...
@pytest.mark.parametrize(
    ...
    [
        ...
        {
            "name": "error_with_continuation",
            "content": "hello",
            "input": "hallo",
            "expected_accuracy": 80.0,
            "expected_efficiency": 100.0,
            "expected_correctness": 80.0,
            "expected_errors": 1,
        },
        {
            ...
            "content": "python",
            "input": "pythin",
            "expected_accuracy": 83.33,
            "expected_efficiency": 100.0,
            "expected_correctness": 83.33,
            "expected_errors": 1,
        },
        {
            "name": "backspace_multiple",
            "content": "code",
            "input": "cxo\bd\be",
            "expected_accuracy": 20.0,
            "expected_efficiency": 80.0,
            "expected_correctness": 25.0,
            "expected_errors": 2,
        },
...
def test_typing_input_handling(app: QApplication, qtbot: Any, test_case: Dict[str, Any]) -> None:
    ...
    with patch("time.time") as mock_time:
        ...
        assert accuracy_delta < 0.1, (
            "Test case '{}': Accuracy should be close to {}%, got {}%".format(
                test_case["name"], expected_accuracy, stats["accuracy"]
            )
        )
        assert efficiency_delta < 0.1, (
            "Test case '{}': Efficiency should be close to {}%, got {}%".format(
                test_case["name"], expected_efficiency, stats["efficiency"]
            )
        )
        assert correctness_delta < 0.1, (
            "Test case '{}': Correctness should be close to {}%, got {}%".format(
                test_case["name"], expected_correctness, stats["correctness"]
            )
        )
        assert stats["errors"] == expected_errors, (
            "Test case '{}': Expected {} errors, got {}".format(
...
def test_session_completion(
    app: QApplication, qtbot: Any, mock_session_manager: SessionManager
) -> None:
    ...
    with patch("datetime.datetime") as mock_datetime, patch("time.time") as mock_time:
        ...
        # Move time forward to simulate typing duration
        mock_time.return_value += 5.0  # 5 seconds later
        mock_datetime.now.return_value += datetime.timedelta(seconds=5)

        # Simulate typing the rest of the content
        ...
        with patch.object(screen, "_calculate_stats", return_value=stats):
            ...
            assert abs(screen.completion_dialog.stats["wpm"] - 12.0) < 0.1, (
                "WPM should match expected value"
            )
            assert screen.completion_dialog.stats["accuracy"] == 100.0, (
                "Accuracy should match expected value"
            )
            assert screen.completion_dialog.stats["efficiency"] == 100.0, (
                "Efficiency should match expected value"
            )
            assert screen.completion_dialog.stats["correctness"] == 100.0, (
                "Correctness should match expected value"
            )
            assert screen.completion_dialog.stats["total_time"] == 5.0, (
                "Total time should match expected value"
            )
            assert screen.completion_dialog.stats["expected_chars"] == 4, (
                "Expected chars should match content length"
            )
            assert screen.completion_dialog.stats["actual_chars"] == 4, (
                "Actual chars should match expected value"
            )
            assert screen.completion_dialog.stats["correct_chars"] == 4, (
                "Correct chars should match expected value"
            )
            assert screen.completion_dialog.stats["errors"] == 0, (
                "Session should have correct error count"
            )

            # Patch the save_session_data function to avoid import issues
            with patch("models.session_manager.save_session_data", return_value=True):
            ...
            assert isinstance(session_arg, Session), "Should pass a Session object"
            ...
            assert session_arg.total_time == 5.0, "Session should have correct total time"
            assert session_arg.session_wpm == 12.0, "Session should have correct WPM"
            assert session_arg.session_cpm == 24.0, "Session should have correct CPM"
            assert session_arg.expected_chars == 4, (
                "Session should have correct expected char count"
            )
            ...
            if hasattr(session_arg, "correctness"):
                assert session_arg.correctness == 1.0, (
                    "Session should have correct correctness (1.0)"
                )


@pytest.mark.qt_no_flask
def test_save_session(app: QApplication, mock_session_manager: SessionManager, qtbot: Any) -> None:
    ...
    assert abs(screen.completion_dialog.stats["wpm"] - 24.0) < 0.1, (
        "WPM should be displayed correctly"
    )
    assert screen.completion_dialog.stats["accuracy"] == 100.0, (
        "Accuracy should be displayed correctly"
    )
    ...
    assert isinstance(call_args, Session), "Should pass a Session object"
...
@pytest.mark.qt_no_flask
def test_only_one_session_saved_on_close(mock_session_manager: SessionManager) -> None:
...
def insert_typing_session(
    drill: TypingDrillScreen, content: str, keystroke_scenario: KeystrokeScenario
) -> int:
...
def test_practice_session_persistence(
    mock_typing_drill: TypingDrillScreen,
    in_memory_db: sqlite3.Connection,
    scenario: KeystrokeScenario,
) -> None:
    ...
    assert (
        abs(session_row[4] - scenario.expected_efficiency / 100.0) < 0.01
    )  # efficiency (convert from percentage)
    assert (
        abs(session_row[5] - scenario.expected_correctness / 100.0) < 0.01
    )  # correctness (convert from percentage)
    assert (
        abs(session_row[6] - scenario.expected_accuracy / 100.0) < 0.01
    )  # accuracy (convert from percentage)


@pytest.mark.parametrize("scenario", KEYSTROKE_SCENARIOS, ids=[s.name for s in KEYSTROKE_SCENARIOS])
def test_keystroke_persistence(
    mock_typing_drill: TypingDrillScreen,
    in_memory_db: sqlite3.Connection,
    scenario: KeystrokeScenario,
) -> None:
...
def test_error_records_persistence(
    mock_typing_drill: TypingDrillScreen,
    in_memory_db: sqlite3.Connection,
    scenario: KeystrokeScenario,
) -> None:
    ...
    error_count = cursor.execute(
        "SELECT COUNT(*) FROM session_errors WHERE session_id=?", (session_id,)
    ).fetchone()[0]
...
def test_backspace_handling(
    mock_typing_drill: TypingDrillScreen, in_memory_db: sqlite3.Connection
) -> None:
    ...
    session_id = insert_typing_session(
        mock_typing_drill, backspace_scenario.content, backspace_scenario
    )
    ...
    assert backspace_errors == 5, (
        f"All backspaces should be marked as errors, got {backspace_errors}"
    )
    ...
    expected_sequence = [
        ...
        ("x", 1, 4),
        ("\b", 1, 4),
        (" ", 0, 4),  # Mistake and correct
        ("t", 0, 5),
        ("e", 0, 6),  # " te"
        ("\b", 1, 6),
        ("\b", 1, 5),  # Backspace twice
        ("x", 1, 4),
        ("y", 1, 5),  # More mistakes
        ("\b", 1, 5),
        ("\b", 1, 4),  # Backspace twice again
        ("t", 0, 4),
        ("e", 0, 5),
        ("x", 0, 6),
        ("t", 0, 7),  # Correct "text"
    ...
    for i, (char, is_error, pos) in enumerate(keystrokes):
        ...
        assert is_error == exp_error, (
            f"Keystroke {i} (char '{char}'): expected error={exp_error}, got {is_error}"
        )
        assert pos == exp_pos, (
            f"Keystroke {i} (char '{char}'): expected position {exp_pos}, got {pos}"
        )
...
@pytest.mark.qt_no_flask
def test_two_sessions_saved_on_retry(
    app: QApplication, qtbot: Any, mock_session_manager: SessionManager
) -> None:
    ...
    with patch("models.session_manager.save_session_data", return_value=True):
        ...
        assert second_session_id != first_session_id, (
            "Second session should have a different ID than the first session"
        )
        ...
        assert second_session.efficiency == 0.95, (
            "Second session should have efficiency of 0.95 (95%)"
        )
        assert second_session.correctness == 0.8, (
            "Second session should have correctness of 0.8 (80%)"
        )

        # Verify both sessions were saved
        assert mock_session_manager.create_session.call_count == 2, (
            "Two distinct sessions should be saved"
        )
