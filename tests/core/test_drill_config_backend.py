...
"""
import pytest
from services.drill_config_service import DrillConfigService
...
@pytest.fixture
def drill_cfg_service(temp_db):
    return DrillConfigService(temp_db)

def test_get_categories(drill_cfg_service):
    cats = drill_cfg_service.get_categories()
    assert len(cats) == 2
    assert cats[0].category_name == 'Alpha'
    assert cats[1].category_name == 'Beta'

def test_get_snippets_by_category(drill_cfg_service):
    snips = drill_cfg_service.get_snippets_by_category(1)
    assert len(snips) == 2
    assert snips[0].snippet_name == 'First'
    assert snips[1].snippet_name == 'Second'
    snips2 = drill_cfg_service.get_snippets_by_category(2)
    assert len(snips2) == 1
    assert snips2[0].snippet_name == 'Other'

def test_get_session_info(drill_cfg_service):
    info = drill_cfg_service.get_session_info(1)
    assert info.last_start_index == 4  # Most recent session
    assert info.last_end_index == 7
    assert info.snippet_length == 7  # 'abc' + 'defg'
    info2 = drill_cfg_service.get_session_info(2)
    assert info2.last_start_index is None
    assert info2.last_end_index is None
    assert info2.snippet_length == 3

def test_handles_missing_data(drill_cfg_service):
    # Category with no snippets
    snips = drill_cfg_service.get_snippets_by_category(999)
    assert snips == []
    # Snippet with no parts or sessions
    info = drill_cfg_service.get_session_info(999)
