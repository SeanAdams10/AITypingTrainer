# tests/api/test_category_api.py
# (To be rebuilt: Pytest-based API tests for category endpoints)
import pytest
import urllib.parse
from flask import Flask
from db.database_manager import DatabaseManager
from api.category_api import create_category_api
...
def app(tmp_path):
    ...
    app = Flask(__name__)
    app.register_blueprint(create_category_api())
...
def test_create_category(client, name, expected_status):
    ...
    if expected_status == 201:
        ...
        assert data["category"]["category_name"] == name
    else:
...
def test_create_duplicate_category(client):
    resp1 = client.post("/api/categories", json={"category_name": "DupCat"})
    assert resp1.status_code == 201
    resp2 = client.post("/api/categories", json={"category_name": "DupCat"})
