...
def app(tmp_path, monkeypatch):
    ...
    tables = db.execute_query("SELECT name FROM sqlite_master WHERE type='table' AND name='categories'")
    assert tables, 'categories table does not exist in test DB after initialization!'
...
def client(app):
    ...
    db.execute("INSERT INTO categories (category_id, category_name) VALUES (1, 'Alpha') ON CONFLICT(category_id) DO NOTHING", commit=True)
    return app.test_client()

@pytest.fixture
def category_id(client):
    # Create a category directly using the CategoryManager
    print("Creating test category directly via CategoryManager...")
    try:
        db_manager = DatabaseManager()
        category_manager = CategoryManager(db_manager)
        category = category_manager.create_category("APIcat")
...
def test_api_snippet_sql_injection(client, category_id):
    ...
    data = resp.get_json()
    assert data["success"] is False
    assert "dangerous" in data["message"] or "forbidden" in data["message"] or "invalid" in data["message"]
