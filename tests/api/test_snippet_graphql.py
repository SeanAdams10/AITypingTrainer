...
def graphql(
    client: FlaskClient, query: str, variables: Optional[Dict[str, Any]] = None
) -> Response:
    """
...
def test_graphql_create_snippet(client: FlaskClient, category_id: int) -> None:
    ...
    query = f"""
    mutation {{
      createSnippet(categoryId: {category_id}, snippetName: "Test", content: "abc") {{
        snippet {{ snippetId }}
      }}
    }}
    """
...
def test_graphql_list_snippets(client: FlaskClient, category_id: int) -> None:
    ...
    query_create = (
        f"""mutation {{ createSnippet(categoryId: {category_id}, snippetName: \"Test\", "
        f"content: \"abc\") {{ snippet {{ snippetId }} }} }}"""
    )
    client.post("/api/graphql", json={"query": query_create})
    # Now list
    query = (
        f"""query {{ snippets(categoryId: {category_id}) {{ snippetId snippetName content }} }}"""
    )
    resp = client.post("/api/graphql", json={"query": query})
    assert resp.status_code == 200
    json_data = cast(Dict[str, Any], resp.json)
...
def test_graphql_variables_style(client: FlaskClient) -> None:
    ...
    variables = {
        "categoryId": 1,
        "snippetName": "Variables Style",
        "content": "Using GraphQL variables",
    }
    ...
    assert json_data["data"]["createSnippet"]["snippet"]["snippetId"] is not None
    assert (
        json_data["data"]["createSnippet"]["snippet"]["snippetName"]
        == "Variables Style"
    )
