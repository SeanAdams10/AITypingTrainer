...
def test_graphql_list_categories(
    client: FlaskClient, category_1: int, category_2: int
) -> None:
    """
...
def test_graphql_create_category(client: FlaskClient) -> None:
    ...
    query = """
    mutation {
      createCategory(categoryName: "New Test Category") {
        category {
          categoryId
          categoryName
        }
    ...
    assert "errors" not in json_data, f"GraphQL errors: {json_data.get('errors')}"
    assert (
        json_data["data"]["createCategory"]["category"]["categoryName"]
        == "New Test Category"
    )
...
def test_graphql_update_category(client: FlaskClient, category_1: int) -> None:
    ...
    query = f"""
    mutation {{
      updateCategory(categoryId: {category_1}, categoryName: \"{new_name}\") {{
        category {{
          categoryId
          categoryName
        }}
...
def test_graphql_create_duplicate_category(
    client: FlaskClient, category_1: int
) -> None:
    """
    ...
    query = f"""
    mutation {{
      createCategory(categoryName: \"{existing_name}\") {{
        category {{
          categoryId
        }}
      }}
