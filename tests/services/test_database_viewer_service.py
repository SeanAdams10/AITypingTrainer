...
def db_manager(tmp_path) -> Generator[DatabaseManager, None, None]:
    ...
    db_manager.execute("INSERT INTO categories (category_name) VALUES (?)", ("Test Category 1",))
    db_manager.execute("INSERT INTO categories (category_name) VALUES (?)", ("Test Category 2",))
    db_manager.execute("INSERT INTO categories (category_name) VALUES (?)", ("Another Category",))

    # Add sample data for snippets
    cat1_id = 1
    cat2_id = 2
    db_manager.execute(
        "INSERT INTO snippets (category_id, snippet_name) VALUES (?, ?)", (cat1_id, "Snippet 1")
    )
    db_manager.execute(
        "INSERT INTO snippets (category_id, snippet_name) VALUES (?, ?)", (cat1_id, "Snippet 2")
    )
    db_manager.execute(
...
class TestDatabaseViewerService:
    ...
    def test_get_table_data_filtering(self, service: DatabaseViewerService) -> None:
        ...
        data = service.get_table_data(
            "categories", filter_column="category_name", filter_value="Another"
        )
        assert len(data["rows"]) == 1
        assert data["rows"][0]["category_name"] == "Another Category"
    ...
    def test_get_table_data_invalid_parameters(self, service: DatabaseViewerService) -> None:
        ...
        with pytest.raises(InvalidParameterError):
            service.get_table_data(
                "categories", filter_column="nonexistent_column", filter_value="test"
            )
    ...
    def test_export_table_data_to_csv(self, service: DatabaseViewerService) -> None:
        ...
        service.export_table_to_csv(
            "categories", output, filter_column="category_name", filter_value="Another"
        )
        output.seek(0)
