...
class TestNGramAnalyzer:
    ...
    class DbManagerMock:
        ...
        def get_instance(self) -> "TestNGramAnalyzer.DbManagerMock":
            return self
    ...
    @pytest.fixture
    def db_mock(self, test_db_path: Path) -> "TestNGramAnalyzer.DbManagerMock":
        """
    ...
    def patched_analyzer(
        self,
        db_mock: "TestNGramAnalyzer.DbManagerMock",
        monkeypatch: pytest.MonkeyPatch,
    ) -> Callable[[int], NGramAnalyzer]:
        """
        ...
        def _create_analyzer(n_size: int) -> NGramAnalyzer:
            ...
            analyzer.db_manager = db_mock  # type: ignore
            return analyzer
    ...
    def test_whitespace_exclusion(
        self,
        setup_database: sqlite3.Connection,
        patched_analyzer: Callable[[int], NGramAnalyzer],
        test_db_path: Path,
    ) -> None:
        ...
        keystrokes = [
            ...
            {"expected_char": " ", "is_correct": True, "time_since_previous": 120},
        ...
        with sqlite3.connect(str(test_db_path)) as verify_conn:
            ...
            cursor.execute(f"SELECT * FROM {analyzer.SPEED_TABLE} WHERE ngram_size = 3")
            ...
            assert (
                "e c" not in ngram_texts
            ), "N-gram 'e c' with space should be excluded"
            assert "cat" in ngram_texts, "Expected 'cat' n-gram to be included"

    def test_speed_accuracy_requirement(
        self,
        setup_database: sqlite3.Connection,
        patched_analyzer: Callable[[int], NGramAnalyzer],
        test_db_path: Path,
        ...
        keystrokes = [
            {"expected_char": "t", "is_correct": True, "time_since_previous": 100},
            {"expected_char": "h", "is_correct": True, "time_since_previous": 110},
            {"expected_char": "e", "is_correct": False, "time_since_previous": 90},
        ...
        with sqlite3.connect(str(test_db_path)) as verify_conn:
            ...
            assert (
                "the" not in speed_ngram_texts
            ), "N-gram 'the' with an error should not be in speed analysis"
            cursor.execute(f"SELECT * FROM {analyzer.ERROR_TABLE} WHERE ngram_size = 3")
            error_ngrams = cursor.fetchall()
            error_ngram_texts = [row["ngram_text"] for row in error_ngrams]
            assert (
                "the" in error_ngram_texts
            ), "N-gram 'the' with an error should be in error analysis"

    def test_error_last_char_requirement(
        self,
        setup_database: sqlite3.Connection,
        patched_analyzer: Callable[[int], NGramAnalyzer],
        test_db_path: Path,
        ...
        keystrokes = [
            {"expected_char": "t", "is_correct": False, "time_since_previous": 100},
            {"expected_char": "h", "is_correct": True, "time_since_previous": 110},
            {"expected_char": "e", "is_correct": True, "time_since_previous": 90},
            {"expected_char": "c", "is_correct": True, "time_since_previous": 120},
            {"expected_char": "a", "is_correct": True, "time_since_previous": 100},
            {"expected_char": "t", "is_correct": False, "time_since_previous": 95},
        ]
        self._add_keystrokes(1, keystrokes, conn)
        ...
        with sqlite3.connect(str(test_db_path)) as verify_conn:
            ...
            assert (
                "the" not in error_texts
            ), "N-gram 'the' should not be in error table since error is not in last position"
            assert (
                "cat" in error_texts
            ), "N-gram 'cat' should be in error table since error is in last position"

    def test_all_ngram_sizes(
        self,
        setup_database: sqlite3.Connection,
        patched_analyzer: Callable[[int], NGramAnalyzer],
        test_db_path: Path,
        ...
        for n_size in valid_sizes:
            ...
            with sqlite3.connect(str(test_db_path)) as verify_conn:
                ...
                if len(keystrokes) < n_size:
                    assert (
                        count == 0
                    ), f"No n-grams should be found for size {n_size} with insufficient keystrokes"
                else:
                ...
                for text in ngram_texts:
                    assert (
                        len(text) == n_size
                    ), f"N-gram '{text}' has incorrect length for size {n_size}"

    def test_specific_ngram_scenarios(
        self,
        setup_database: sqlite3.Connection,
        patched_analyzer: Callable[[int], NGramAnalyzer],
        test_db_path: Path,
    ...
    def test_bigrams(
        ...
        with sqlite3.connect(str(test_db_path)) as verify_conn:
            verify_conn.row_factory = sqlite3.Row
            cursor = verify_conn.cursor()
            cursor.execute(f"SELECT * FROM {analyzer.SPEED_TABLE} WHERE ngram_size = 2")
    ...
    def test_get_slow_ngrams(
        ...
        keystrokes = [
            ...
            {"expected_char": "h", "is_correct": True, "time_since_previous": 110},
            {"expected_char": "e", "is_correct": True, "time_since_previous": 90},
            {"expected_char": "t", "is_correct": True, "time_since_previous": 200},
            {"expected_char": "h", "is_correct": True, "time_since_previous": 250},
        ...
        for i in range(1, len(slow_ngrams)):
            assert (
                slow_ngrams[i - 1]["avg_time"] >= slow_ngrams[i]["avg_time"]
            ), "N-grams not sorted by avg_time"
        ...
        for ngram in slow_ngrams:
            ...
            assert (
                ngram["ngram_size"] == n_size
            ), f"N-gram size mismatch: {ngram['ngram_size']} != {n_size}"

    def test_get_error_ngrams(
        self,
        setup_database: sqlite3.Connection,
        patched_analyzer: Callable[[int], NGramAnalyzer],
    ) -> None:
        ...
        keystrokes = [
            ...
            {"expected_char": "c", "is_correct": True, "time_since_previous": 100},
            {"expected_char": "a", "is_correct": True, "time_since_previous": 95},
            {"expected_char": "t", "is_correct": False, "time_since_previous": 90},
        ]
        self._add_keystrokes(1, keystrokes, conn)
        ...
        for i in range(1, len(error_ngrams)):
            assert (
                error_ngrams[i - 1]["count"] >= error_ngrams[i]["count"]
            ), "N-grams not sorted by count"
    ...
    def test_create_ngram_snippet(
        ...
        def mock_create_snippet_from_ngrams(
        ...
        monkeypatch.setattr(
            PracticeGenerator,
            "create_snippet_from_ngrams",
            mock_create_snippet_from_ngrams,
        )
        ...
        keystrokes = [
            ...
            {"expected_char": "c", "is_correct": True, "time_since_previous": 90},
            {"expected_char": "a", "is_correct": True, "time_since_previous": 95},
            {"expected_char": "t", "is_correct": False, "time_since_previous": 300},
        ]
        self._add_keystrokes(1, keystrokes, conn)
        ...
        snippet = analyzer.create_ngram_snippet(
            ngram_type="slow", name="Slow Bigram Practice", count=5
        )
        ...
        error_snippet = analyzer.create_ngram_snippet(
            ngram_type="error", name="Error Bigram Practice", count=5
        )
        ...
        assert (
            error_snippet["name"] == "Error Bigram Practice"
        ), "Error snippet name doesn't match"

    def test_record_keystrokes_ngram_limits(
        self,
        setup_database: sqlite3.Connection,
        patched_analyzer: Callable[[int], NGramAnalyzer],
    ) -> None:
        ...
        for n_size in range(2, 11):
            cursor = conn.cursor()
            cursor.execute(
                f"DELETE FROM session_ngram_speed WHERE ngram_size = {n_size}"
            )
            cursor.execute(
                f"DELETE FROM session_ngram_error WHERE ngram_size = {n_size}"
            )
            conn.commit()

            for seq_length in range(1, n_size + 2):
                ...
                if seq_length < n_size:
                    assert (
                        count == 0
                    ), f"No n-grams should be generated for sequences shorter than {n_size}"
                else:
                    expected_count = seq_length - n_size + 1
                    assert (
                        count == expected_count
                    ), f"Expected {expected_count} n-grams for sequence of length {seq_length}"
