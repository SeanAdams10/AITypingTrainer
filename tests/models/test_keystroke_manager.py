...
def sample_keystroke() -> Keystroke:
    ...
    return Keystroke(
        session_id=str(uuid.uuid4()),  # Use string UUID for session ID
        keystroke_id=str(uuid.uuid4()),  # Use string UUID for keystroke ID
...
def test_session() -> str:
    ...
    return str(uuid.uuid4())  # Return a new UUID as string
    ...
    keystrokes: List[Dict[str, Any]] = [
        {
            ...
            "keystroke_id": str(uuid.uuid4()),  # Generate a new UUID string for keystroke ID
...
def test_delete_keystrokes_by_session(
    ...
    try:
        print(f"Calling delete_keystrokes_by_session with ID: {session_id}", file=sys.stderr)
