def test_four_keystrokes_no_errors(self, temp_db, test_practice_session, four_keystrokes_no_errors):
    ...
    # Use a safer approach to checking for no error n-grams - instead of assuming
    # the dictionary keys exist, we check if the analyzer has error_ngrams at all
    # and then check any specific n-gram sizes that do exist
    assert hasattr(analyzer, 'error_ngrams'), "Analyzer should have error_ngrams attribute"
    
    # Check any existing error n-grams size dictionaries have empty lists
    for ngram_size in [2, 3, 4]:
        if ngram_size in analyzer.error_ngrams:
            assert len(analyzer.error_ngrams[ngram_size]) == 0, f"Should be no error {ngram_size}-grams"
