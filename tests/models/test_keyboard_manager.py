...
def keyboard_manager(db_with_tables: DatabaseManager) -> KeyboardManager:
    return KeyboardManager(db_manager=db_with_tables)
...
def test_get_keyboard(keyboard_manager: KeyboardManager, test_user: User) -> None:
    ...
    keyboard_manager.save_keyboard(keyboard=k)
    fetched = keyboard_manager.get_keyboard_by_id(keyboard_id=k.keyboard_id)
...
def test_update_keyboard_name(keyboard_manager: KeyboardManager, test_user: User) -> None:
    ...
    assert keyboard_manager.save_keyboard(keyboard=k)
    updated = keyboard_manager.get_keyboard_by_id(keyboard_id=k.keyboard_id)
...
def test_update_keyboard_target_speed(keyboard_manager: KeyboardManager, test_user: User) -> None:
    ...
    saved = keyboard_manager.get_keyboard_by_id(keyboard_id=k.keyboard_id)
...
def test_delete_keyboard(keyboard_manager: KeyboardManager, test_user: User) -> None:
    ...
    keyboard_manager.save_keyboard(keyboard=k)
    assert keyboard_manager.delete_keyboard(keyboard_id=k.keyboard_id)
    with pytest.raises(KeyboardNotFound):
        keyboard_manager.get_keyboard_by_id(keyboard_id=k.keyboard_id)
