...
# @pytest.fixture
# def three_keystrokes_error_at_second(
#     temp_db: DatabaseManager, test_practice_session: Session
# ) -> List[Keystroke]:
#     """
...
# @pytest.fixture
# def three_keystrokes_error_at_third(
#     temp_db: DatabaseManager, test_practice_session: Session
# ) -> List[Keystroke]:
#     """
...
#         temp_db: DatabaseManager,
#         test_practice_session: Session,
#         single_keystroke: List[Keystroke],
#     ) -> None:
#         """
#         Test objective: Verify that a single keystroke produces no n-grams.

#         This test checks that:
#         1. The manager properly handles a single keystroke scenario
#         2. No n-grams are identified in the manager object
#         3. No n-grams are saved to the database
#         """
#         # Define the session ID constant for better readability in assertions
#         session_id = test_practice_session.session_id

#         # Create NGramManager instance with a single keystroke
#         manager = NGramManager(temp_db)
#         manager.session = test_practice_session
#         manager.keystrokes = single_keystroke
#         manager.analyze()
...
#         save_result = manager.save_to_database()
...
#         temp_db: DatabaseManager,
#         test_practice_session: Session,
#         two_keystrokes_no_errors: List[Keystroke],
#     ) -> None:
#         """
#         Test objective: Verify that two keystrokes produce a single bigram with correct timing.

#         This test checks that:
#         1. The manager properly handles two keystrokes scenario
...
#         # Create NGramManager instance with two keystrokes
#         manager = NGramManager(temp_db)
#         manager.session = test_practice_session
#         manager.keystrokes = two_keystrokes_no_errors
#         manager.analyze()
...
#         assert len(manager.speed_ngrams[2]) == 1, "Should be exactly one speed bigram"

#         # Retrieve the single bigram
#         bigram_text = "Th"  # The expected bigram text

#         # Validate its properties
#         bigram = _find_ngram_in_list(manager.speed_ngrams[2], bigram_text)
...
#         assert len(manager.error_ngrams.get(2, [])) == 0, "Should be no error bigrams of size 2"
...
#         # Check that the saved time value is the average time per character
#         # (500ms / 2 chars = 250ms)
...
#         temp_db: DatabaseManager,
#         test_practice_session: Session,
#         two_keystrokes_error_at_first: List[Keystroke],
#     ) -> None:
#         """
#         Test objective: Verify that two keystrokes with an error on the first keystroke
#         are analyzed correctly.

#         This test checks that:
#         1. The manager properly handles a scenario with an error on the first keystroke
...
#         # Create NGramManager instance with two keystrokes, first has error
#         manager = NGramManager(temp_db)
#         manager.session = test_practice_session
#         manager.keystrokes = two_keystrokes_error_at_first
#         manager.analyze()
...
#         error_bigrams = manager.get_most_error_prone_ngrams(size=2)
...
#         temp_db: DatabaseManager,
#         test_practice_session: Session,
#         two_keystrokes_error_at_second: List[Keystroke],
#     ) -> None:
#         """
#         Test objective: Verify that two keystrokes with an error on the second keystroke
#         are analyzed correctly.

#         This test checks that:
#         1. The manager properly handles a scenario with an error on the second keystroke
...
#         # Create NGramManager instance with two keystrokes, second has error
#         manager = NGramManager(temp_db)
#         manager.session = test_practice_session
#         manager.keystrokes = two_keystrokes_error_at_second
#         manager.analyze()
...
#         error_bigrams_list = manager.error_ngrams.get(2, [])
...
#         temp_db: DatabaseManager,
#         test_practice_session: Session,
#         three_keystrokes_no_errors: List[Keystroke],
#     ) -> None:
#         """
#         Test objective: Verify that three keystrokes produce correct bigrams and trigram
#         with proper timing.

#         This test checks that:
#         1. The manager properly handles three keystrokes scenario
...
#         # Create NGramManager instance with three keystrokes
#         manager = NGramManager(temp_db)
#         manager.session = test_practice_session
#         manager.keystrokes = three_keystrokes_no_errors
#         manager.analyze()
...
#         assert 2 in manager.speed_ngrams, "Speed n-grams dictionary should have key for bigrams"
#         assert len(manager.speed_ngrams[2]) == 2, "Should be exactly two speed bigrams"

#         # Validate the first bigram 'Th'
#         bigram1_text = "Th"  # The expected first bigram text

#         # Retrieve the bigram using the helper function
#         bigram1 = _find_ngram_in_list(manager.speed_ngrams[2], bigram1_text)
...
#         bigram2 = _find_ngram_in_list(manager.speed_ngrams[2], bigram2_text)
...
#         assert 3 in manager.speed_ngrams, "Speed n-grams dictionary should have key for trigrams"
#         assert len(manager.speed_ngrams[3]) == 1, "Should be exactly one speed trigram"

#         # Validate the trigram 'The'
#         trigram_text = "The"  # The expected trigram text

#         # Retrieve the trigram from the dictionary
#         trigram = _find_ngram_in_list(manager.speed_ngrams[3], trigram_text)
...
#         assert len(manager.error_ngrams[2]) == 0, "Should be no error bigrams"
#         assert len(manager.error_ngrams[3]) == 0, "Should be no error trigrams"
...
#     def test_basic_ngram_manager_initialization(self, temp_db: DatabaseManager) -> None:
#         """
#         Test objective: Verify NGramManager can be initialized and returns correct slowest n-grams and error n-grams from DB.
#         This test inserts mock n-gram data for 3 sessions and checks the query interface.
#         """
#         import uuid
#         from datetime import datetime, timedelta
#         # Insert mock category (required for snippet FK)
#         category_id = str(uuid.uuid4())
#         temp_db.execute(
#             "INSERT INTO categories (category_id, category_name) VALUES (?, ?)",
#             (category_id, "Test Category"),
#         )
#         # Insert mock snippet (required for session FK)
#         snippet_id = str(uuid.uuid4())
#         temp_db.execute(
#             "INSERT INTO snippets (snippet_id, category_id, snippet_name) VALUES (?, ?, ?)",
#             (snippet_id, category_id, "Test Snippet"),
#         )
#         # Insert 3 sessions and n-gram data for each
...
#                 ),
#             )
#             # Insert two bigrams for each session
#             temp_db.execute(
#                 "INSERT INTO session_ngram_speed (ngram_speed_id, session_id, ngram_size, ngram_text, ngram_time_ms) VALUES (?, ?, ?, ?, ?)",
#                 (str(uuid.uuid4()), session_id, 2, "Th", 500),
#             )
#             temp_db.execute(
#                 "INSERT INTO session_ngram_speed (ngram_speed_id, session_id, ngram_size, ngram_text, ngram_time_ms) VALUES (?, ?, ?, ?, ?)",
#                 (str(uuid.uuid4()), session_id, 2, "he", 1000),
#             )
#             # Insert one error bigram for each session
#             temp_db.execute(
#                 "INSERT INTO session_ngram_errors (ngram_error_id, session_id, ngram_size, ngram_text) VALUES (?, ?, ?, ?)",
#                 (str(uuid.uuid4()), session_id, 2, "he"),
#             )
#         # Now test NGramManager
#         manager = NGramManager(temp_db)
#         # Test slowest_n (should return both bigrams, slowest first)
#         slowest = manager.slowest_n(2, ngram_sizes=[2], lookback_distance=10)
#         assert len(slowest) == 2, "Should return two slowest bigrams"
#         assert slowest[0].ngram == "he", "Slowest bigram should be 'he' (1000ms)"
#         assert slowest[1].ngram == "Th", "Second slowest bigram should be 'Th' (500ms)"
#         # Test error_n (should return one error bigram)
#         errors = manager.error_n(2, ngram_sizes=[2], lookback_distance=10)
#         assert len(errors) == 1, "Should return one error bigram"
#         assert errors[0].ngram == "he", "Error bigram should be 'he'"
