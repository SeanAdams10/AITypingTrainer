...
class TestNGramModelsExtended:
    ...
    def test_four_keystrokes_no_errors(
        ...
        """
        Test objective: Verify that four correct keystrokes ('T', 'h', 'e', 'n') are analyzed correctly.

        This test checks a scenario where:
        - Four correct keystrokes are entered in sequence
        - Specific timing between keystrokes: 500ms, 1000ms, 300ms
        - Expected outcomes:
          - Six n-grams generated (3 bigrams, 2 trigrams, 1 quadrigram)
          - All n-grams have correct timing and no errors
        """
        # 0. Create an NGramAnalyzer instance with the keystrokes
        ...
        assert len(analyzer.speed_ngrams) == 2, (
            f"Expected 2 n-gram sizes, got {len(analyzer.speed_ngrams)}"
        )

        # Check that there are 2 bigrams and 1 trigram
        assert len(analyzer.speed_ngrams[2]) == 2, (
            f"Expected 2 bigrams, got {len(analyzer.speed_ngrams[2])}"
        )
        assert len(analyzer.speed_ngrams[3]) == 1, (
            f"Expected 1 trigram, got {len(analyzer.speed_ngrams[3])}"
        )
        assert len(analyzer.error_ngrams) == 0, (
            f"Expected 0 error n-grams, got {len(analyzer.error_ngrams)}"
        )
        ...
        assert he_ngram.total_time_ms == 1000, (
            f"Expected 'he' time to be 1000ms, got {he_ngram.total_time_ms}ms"
        )
        ...
        assert en_ngram.total_time_ms == 300, (
            f"Expected 'en' time to be 300ms, got {en_ngram.total_time_ms}ms"
        )
        ...
        assert hen_ngram.total_time_ms == 1300, (
            f"Expected 'hen' time to be 1300ms, got {hen_ngram.total_time_ms}ms"
        )
        ...
        for row in speed_ngrams_db:
            ...
            if ngram == "he":
                ...
                assert 430 <= time_ms <= 435, (
                    f"'hen' average time should be ~433.33ms (1300/3), got {time_ms}"
                )
            else:
    ...
    def test_four_keystrokes_error_at_first(
        ...
        This test checks a scenario where:
        - Four keystrokes: G(T), h, e, n. Times: 0, 500, 1000, 300
        - Expected outcomes:
          - Six n-grams generated (3 bigrams, 2 trigrams, 1 quadrigram)
          - Specific n-grams should have errors marked and error details populated
        """
        # 0. Create an NGramAnalyzer instance with the keystrokes
        ...
        # Check specific n-grams and their timings
        # Bigrams
        gh_ngram = _find_ngram_in_list(analyzer.speed_ngrams[2], "Gh")
        assert gh_ngram is not None, "Bigram 'Gh' not found in speed n-grams"
        assert gh_ngram.total_time_ms == 250, (
            f"Expected 'Gh' time to be 250ms, got {gh_ngram.total_time_ms}ms"
        )
        assert gh_ngram.size == 2, f"Expected 'Gh' size to be 2, got {gh_ngram.size}"

        he_ngram = _find_ngram_in_list(analyzer.speed_ngrams[2], "he")
        assert he_ngram is not None, "Bigram 'he' not found in speed n-grams"
        assert he_ngram.total_time_ms == 750, (
            f"Expected 'he' time to be 750ms, got {he_ngram.total_time_ms}ms"
        )
        assert he_ngram.size == 2, f"Expected 'he' size to be 2, got {he_ngram.size}"

        en_ngram = _find_ngram_in_list(analyzer.speed_ngrams[2], "en")
        assert en_ngram is not None, "Bigram 'en' not found in speed n-grams"
        assert en_ngram.total_time_ms == 150, (
            f"Expected 'en' time to be 150ms, got {en_ngram.total_time_ms}ms"
        )
        assert en_ngram.size == 2, f"Expected 'en' size to be 2, got {en_ngram.size}"

        # Trigram
        ghe_ngram = _find_ngram_in_list(analyzer.speed_ngrams[3], "Ghe")
        assert ghe_ngram is not None, "Trigram 'Ghe' not found in speed n-grams"
        assert ghe_ngram.total_time_ms == 500, (
            f"Expected 'Ghe' time to be 500ms, got {ghe_ngram.total_time_ms}ms"
        )
        assert ghe_ngram.size == 3, f"Expected 'Ghe' size to be 3, got {ghe_ngram.size}"
        ...
        # Quadrigram
        ghen_ngram = _find_ngram_in_list(analyzer.speed_ngrams[4], "Ghen")
        assert ghen_ngram is not None, "Quadgram 'Ghen' not found in speed n-grams"
        assert ghen_ngram.total_time_ms == 625, (
            f"Expected 'Ghen' time to be 625ms, got {ghen_ngram.total_time_ms}ms"
        )
        assert ghen_ngram.size == 4, f"Expected 'Ghen' size to be 4, got {ghen_ngram.size}"
        ...
        for row in speed_ngrams_db:
            size, ngram, time_ms = row
            if ngram == "Gh":
                assert size == 2, "'Gh' should be size 2"
                assert time_ms == 250, "'Gh' average time should be 250ms"
            elif ngram == "he":
                assert size == 2, "'he' should be size 2"
                assert time_ms == 750, "'he' average time should be 750ms"
            elif ngram == "en":
                assert size == 2, "'en' should be size 2"
                assert time_ms == 150, "'en' average time should be 150ms"
            elif ngram == "Ghe":
                assert size == 3, "'Ghe' should be size 3"
                assert time_ms == 500, "'Ghe' average time should be 500ms"
            elif ngram == "hen":
                assert size == 3, "'hen' should be size 3"
                assert 430 <= time_ms <= 435, (
                    f"'hen' average time should be ~433.33ms (1300/3), got {time_ms}"
                ...
                assert size == 4, "'Ghen' should be size 4"
                assert time_ms == 625, "'Ghen' average time should be 625ms"
            else:
                assert False, f"Unexpected n-gram in database: {ngram}"
    ...
    def test_four_keystrokes_error_at_second(
        ...
        assert len(analyzer.speed_ngrams) > 0, (
            f"Expected at least 1 n-gram size, got {len(analyzer.speed_ngrams)}"
        )
        assert len(analyzer.error_ngrams) > 0, (
            f"Expected at least 1 error n-gram size, got {len(analyzer.error_ngrams)}"
        )
        if 2 in analyzer.error_ngrams:
            assert len(analyzer.error_ngrams[2]) > 0, (
                "No bigram errors found despite having size 2 in error_ngrams"
            )
        # 4. Validate database entries
        speed_ngrams_db = temp_db.fetchall(
            """
    ...
    def test_four_keystrokes_error_at_fourth(
        ...
        assert len(analyzer.error_ngrams) > 0, (
            f"Expected some error n-grams, got {len(analyzer.error_ngrams)}"
        )
        if 2 in analyzer.error_ngrams:
            assert len(analyzer.error_ngrams[2]) > 0, (
                "No bigram errors found despite having size 2 in error_ngrams"
