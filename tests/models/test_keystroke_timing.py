...
def create_keystrokes_with_errors_and_backspaces(text: str) -> List[Dict[str, Any]]:
    ...
    for i in range(
        len(text) + len(error_positions) * 2
    ):  # Extra keystrokes for errors and backspaces
...
def test_first_keystroke_time_is_zero(session_manager, sample_practice_session):
    ...
    # Save the keystroke
    result = session_manager.save_keystrokes(session_id, keystrokes)
    assert result, "Failed to save session data"

    # Verify the keystroke timing
    keystroke_manager = session_manager.db_manager
    ...
    assert saved_keystrokes[0]["time_since_previous"] == 0, (
        "First keystroke should have time_since_previous = 0"
    )
...
def test_short_text_timing(session_manager, sample_practice_session):
    ...
    assert saved_keystrokes[1]["time_since_previous"] > 0, (
        "Second keystroke should have positive timing"
    )
    assert saved_keystrokes[2]["time_since_previous"] > 0, (
        "Third keystroke should have positive timing"
    )
...
def test_fast_typing_timing(session_manager, sample_practice_session):
    ...
    for i in range(1, 4):
        assert 0 < saved_keystrokes[i]["time_since_previous"] < 100, (
            f"Keystroke {i} should have small positive timing for fast typing"
        )
...
def test_slow_typing_timing(session_manager, sample_practice_session):
    ...
    for i in range(1, 4):
        assert saved_keystrokes[i]["time_since_previous"] > 500, (
            f"Keystroke {i} should have large timing value for slow typing"
        )
...
def test_longer_text_timing(session_manager, sample_practice_session):
    ...
    for i in range(min(10, len(saved_keystrokes))):
        print(
            f"Saved keystroke {i}: time_since_previous={saved_keystrokes[i]['time_since_previous']}"
        )
    ...
    assert positive_timings >= len(saved_keystrokes[1:]) * 0.8, (
        "At least 80% of keystrokes should have positive timing"
    )

    # Check total keystrokes matches expected length
    assert len(saved_keystrokes) == len(text), (
        f"Should have {len(text)} keystrokes for the longer text"
    )
...
def test_efficiency_correctness_accuracy_metrics(session_manager, sample_practice_session):
    ...
    session_manager.save_keystrokes(session_id, keystrokes)
