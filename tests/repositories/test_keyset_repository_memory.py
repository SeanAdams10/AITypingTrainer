...
class TestListForKeyboard:
    ...
    def test_list_returns_keysets_for_keyboard(
        self, repo: InMemoryKeysetRepository, keyboard_id: str, sample_keyset: Keyset
    ) -> None:
        """Test listing returns keysets for specific keyboard."""
        repo.save(sample_keyset)
        result = repo.list_for_keyboard(keyboard_id)
        assert len(result) == 1
    ...
    def test_list_excludes_deleted_keysets(
        ...
        repo.save(sample_keyset)
        repo.delete(sample_keyset.keyset_id)  # type: ignore[arg-type]
...
class TestGetById:
    ...
    def test_get_existing_keyset(
        self, repo: InMemoryKeysetRepository, sample_keyset: Keyset
    ) -> None:
...
class TestDelete:
    ...
    def test_delete_existing_keyset(
        ...
        repo.save(sample_keyset)
        result = repo.delete(sample_keyset.keyset_id)  # type: ignore[arg-type]
        assert result is True

    def test_delete_nonexistent_keyset(self, repo: InMemoryKeysetRepository) -> None:
    ...
    def test_delete_is_soft_delete(
        ...
        repo.delete(keyset_id)  # type: ignore[arg-type]

        # Data still in storage but marked deleted
        assert keyset_id in repo._keysets
        assert keyset_id in repo._deleted_ids

    def test_delete_validates_keyset_id(self, repo: InMemoryKeysetRepository) -> None:
...
class TestValidateKeyProgressionUniqueness:
    ...
    def test_validate_checks_multiple_keys(
        ...
        """Test validation checks all provided keys."""
        ks1 = Keyset(
            keyboard_id=keyboard_id,
            keyset_name="P1",
            progression_order=1,
    ...
    def test_validate_ignores_deleted_keysets(
        ...
        repo.save(ks1)
        repo.delete(ks1.keyset_id)  # type: ignore[arg-type]
