...
def api_record_keystroke():
    import sys
    print(f"DEBUG: Incoming keystroke POST data: {request.get_data(as_text=True)}", file=sys.stderr)
    try:
    ...
    if not success:
        ...
        print(f"DEBUG: Keystroke save failed. Exception type: {exc_type}, value: {exc_value}", file=sys.stderr)
        if exc_type is not None and issubclass(exc_type, sqlite3.IntegrityError):
            return make_response(jsonify({'error': 'Duplicate keystroke_id for session or unique constraint violation'}), 400)
