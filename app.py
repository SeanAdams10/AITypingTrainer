...
def keystroke():
    try:
        ...
        required_fields = ['index', 'expected', 'actual', 'correct', 'time']
        for field in required_fields:
            if field not in keystroke:
                return jsonify({"error": f"Missing field in keystroke data: {field}"}), 400

        # Save the keystroke to the database
        try:
            success = Keystroke.save_many(session_id, [keystroke])
            if not success:
                return jsonify({"error": "Failed to save keystroke"}), 500
        except Exception as e:
            print(f"Error saving keystroke: {str(e)}")
            return jsonify({"error": "Failed to save keystroke"}), 500

        return jsonify({"success": True, "message": "Keystroke saved successfully"})

    except Exception as e:
        print(f"Error processing keystroke: {str(e)}")
        return jsonify({"error": str(e)}), 500
